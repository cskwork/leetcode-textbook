(() => {
  // GlyphRainVanilla.ts
  var DEFAULT_CHARSET = "ｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝ0123456789Z*+-<>¦=:.";
  var DEFAULTS = {
...
void main () {
  ...
  function createGlyphRain(elements, options = {}) {
    ...
    let atlasCharset = "";
    function syncAtlas() {
      if (config.charset === atlasCharset)
        return;
    ...
    function onPointerDown(event) {
      if (reducedMotion || stirAmount() <= 0.001)
        return;
      const x = pointerNorm(event);
      if (x < 0)
        return;
  ...
  // entry.js
  globalThis.GlyphRain = { createGlyphRain, supportsHtmlInCanvas };
})();
